...
# ==========================================
DEFAULT_CAMERA_IP = "192.168.0.114"
...
CAMERAS = {
    "cam-01": f"http://{DEFAULT_CAMERA_IP}/ISAPI/Streaming/channels/101/picture",
...
def annotate_frame(frame, zones, zone_occupancy, vehicles, camera_id="?"):
...
def video_feed():
    camera_id = request.args.get('cameraId', '').strip()
    
    # Si recibimos la URL completa (común en el front), mapearla al ID (cam-01, etc)
    if camera_id.startswith('http'):
    ...
    def generate():
        while True:
            ...
            annotated = annotate_frame(frame, zones, occupancy, vehicles, camera_id)
...
def get_parking_status():
    camera_id = request.args.get('cameraId', 'cam-01').strip()
...
def sync_zones():
    data = request.json
    camera_id = data.get('cameraId', 'cam-01').strip()
